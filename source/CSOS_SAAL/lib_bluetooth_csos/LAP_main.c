...
//	wifi_processing_event_send(WIFI_PROCESSING_EVENT_SEND_MQTT, NULL, mqtt_msg);
	...
	uint8_t ack_packet[PAAR_PACKET_ACK_LEN];

	ack_packet[PAAR_PACKET_INDEX_PACKET_TYPE] = LAP_evt_msg.msg[PAAR_PACKET_INDEX_PACKET_TYPE];
	ack_packet[PAAR_PACKET_INDEX_SERVICE_ID] = LAP_evt_msg.msg[PAAR_PACKET_INDEX_SERVICE_ID];
	ack_packet[PAAR_PACKET_INDEX_SEQUENCE_NUM] = 0x11;
	ack_packet[PAAR_PACKET_INDEX_DATA_LEN] = PAAR_PACKET_ACK_DATA_LEN;
	ack_packet[PAAR_PACKET_INDEX_BODY_DATA_CMD] = LAP_evt_msg.msg[PAAR_PACKET_INDEX_BODY_DATA_CMD];
	...
	//send ACK msg...
	LAP_send_ble_msg_central(LAP_evt_msg.conn_handle, profile_data.rx_handle, ack_packet, PAAR_PACKET_ACK_LEN);
#endif
...
#elif(SP_SW_MODE_SETUP == SP_SW_MODE_TEST_PERIPHRAL)
	printf("BLE ADV start\r\n");
	task_sleep(TEST_ADV_START_DELAY);
	LAP_start_ble_adv_LIDx();
#endif
